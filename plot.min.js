...
window.Plot=Plot;function Plot(a,d){this.div=a;this.config=d;this.initialize(a,d);window.addEventListener("resize",function(a){this.initialize(this.div,this.config)}.bind(this))}
Plot.prototype.initialize=function(a,d){this.doWhenLoaded(function(){this.generateCanvas(a);for(var b in d)this[b]=d[b];this.applyDefaultSettings();this.bottomMargin=0;this.ctx=this.canvas.getContext("2d");this.canvas.style["background-color"]=this.backgroundColor;this.calculateDataRanges();this.calculateDrawingRanges();this.calculateDrawingProperties();this.scaleData();this.draw()})};
...
"px";a.innerHTML="";a.appendChild(c);this.canvas=c};
...
yHighlight:[]},b={color:"#fff",shadowColor:"#80e27e",linewidth:4,dataPointRadius:1.5,xHighlight:[],yHighlight:[]},c;for(c in a)void 0==this[c]&&(this[c]=a[c]);a=this.graphs.length;for(c=0;c<a;c++)if("shadow"==this.graphs[c].type)for(var e in b)void 0==this.graphs[c][e]&&(this.graphs[c][e]=b[e]);else for(e in d)void 0==this.graphs[c][e]&&(this.graphs[c][e]=d[e])};
