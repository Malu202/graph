(function(){
'use strict';window.Gauge=Gauge;function Gauge(b,c,d,e,f,g,h){this.initialize(b,c,d,e,f,g,h);this.div=b;this.value=c;this.displayValue=d;this.min=e;this.max=f;this.background=g;this.textColor=h;window.addEventListener("resize",function(b){console.log(this.min);this.initialize(this.div,this.value,this.displayValue,this.min,this.max,this.background,this.textColor);this.changeValue(this.value,this.displayValue)}.bind(this))}
Gauge.prototype.initialize=function(b,c,d,e,f,g,h){this.doWhenLoaded(function(){this.generateCanvas(b);if(null==d||""==d)d=c;this.size=this.height<this.width?this.height:this.width;this.min=e;this.max=f;this.canvas.style["background-color"]=g;this.textColor=h})};
Gauge.prototype.generateCanvas=function(b){var c=window.devicePixelRatio||1,d=b.getBoundingClientRect(),e=document.createElement("canvas");this.width=Math.round(d.width);this.height=Math.round(d.height);e.width=this.width*c;e.height=this.height*c;this.ctx=e.getContext("2d");this.ctx.scale(c,c);b.style.position="relative";b.style.overflow="hidden";b.style.top="0px";b.style.left="0px";e.style.position="absolute";e.style.top="0px";e.style.left="0px";e.style.height=this.height+"px";e.style.width=this.width+
"px";b.innerHTML="";b.appendChild(e);this.canvas=e};
